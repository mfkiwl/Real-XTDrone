...
class Leader:
    ...
    def __init__(self, uav_type, leader_id, uav_num):
        ...
        self.avoid_vel_sub = rospy.Subscriber("/xtdrone/"+uav_type+'_'+str(self.id)+"/avoid_accel", Vector3, self.avoid_accel_callback, queue_size=1)
        ...
        for i in range(self.follower_num):
            rospy.Subscriber('/xtdrone/'+uav_type+'_'+str(i+1)+'/info',String,self.followers_info_callback,i, queue_size=1)
